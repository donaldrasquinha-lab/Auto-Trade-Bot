...
def compute_ema(series: pd.Series, period: int) -> pd.Series:
    """Exponential Moving Average — used for 9-MA and 21-MA."""
    return series.ewm(span=period, adjust=False).mean()

def compute_supertrend(df: pd.DataFrame, period: int, multiplier: float):
    """
    Supertrend indicator.
    Returns Series with values: +1 = uptrend (price above ST), -1 = downtrend.
    """
...
def compute_cpr(df_daily: pd.DataFrame):
    """
    Central Pivot Range (CPR) from the previous day's OHLC.
    Returns dict with pivot, bc (bottom central), tc (top central).
    Narrow CPR = abs(tc - bc) / pivot < 0.002 (0.2%)
    """
    if len(df_daily) < 2:
        return None
    ...
    return {
        ...
        "narrow":    width_pct < 0.20,  # < 0.20% = narrow CPR
    }


def get_daily_candles(ikey: str, days: int = 5) -> pd.DataFrame:
    """Fetch recent daily OHLC for CPR calculation."""
    from datetime import date, timedelta
...
def evaluate_entry_signal(df: pd.DataFrame, cpr: dict, spot: float) -> dict:
    """
    Evaluate all entry conditions on 1-min candle data.
    
    BUY CE conditions (all must be true):
      1. 9-EMA crosses above 21-EMA (price momentum up)
      2. ADX >= 25
      3. +DI >= 25
      4. spot > CPR tc (price above narrow CPR pivot band)
      5. CPR is narrow
      6. Supertrend(9, ATR×1) = +1 (uptrend)
      7. Supertrend(21, ATR×2) = +1 (uptrend)

    BUY PE conditions (all must be true):
      1. 9-EMA crosses below 21-EMA (price momentum down)
      2. ADX >= 25
      3. -DI >= 25
      4. spot < CPR bc (price below narrow CPR pivot band)
      5. CPR is narrow
      6. Supertrend(9, ATR×1) = -1 (downtrend)
      7. Supertrend(21, ATR×2) = -1 (downtrend)

    Returns dict with signal ("CE"/"PE"/"NONE") + all condition flags.
    """
    MIN_BARS = 30   # need enough bars for all indicators
    ...
    ma9_prev_below_21 = bool(ema9.iloc[-2] <= ema21.iloc[-2])  # fresh crossover
    ma_cross_up   = ma9_above_21   # current bar 9>21
    ma_cross_down = not ma9_above_21
...
def _fetch_data():
    ...
    # ══════════════════════════════════════════════
    # ── CPR + entry signal (cached separately — daily candles only needed once) ──
    if not MOCK_MODE:
        try:
            ...
            cpr = None
        try:
            entry_sig = evaluate_entry_signal(df_idx, cpr, live_px)
        except Exception:
            entry_sig = {"signal": "NONE", "reason": "Indicator error"}
    else:
        # Mock CPR and entry signal
        _mock_spot = live_px
        cpr = {
        ...
        entry_sig = evaluate_entry_signal(_mock_df, cpr, live_px)
...
def _frag_buy_trade():
    ...
    MIN_HOLD_SECS = 600   # 10 minutes

    # Read entry signal and CPR from sv_ state
    entry_sig = st.session_state.get("sv_entry_sig") or {"signal":"NONE","reason":"Loading…"}
    cpr       = st.session_state.get("sv_cpr") or {}
    sig       = entry_sig.get("signal", "NONE")   # "CE" / "PE" / "NONE"

    st.subheader("Trade control — mockup")
    st.caption("SL ≈ 50% of target | Min hold 10 min | ATM option | Signal-gated | No real order sent.")
  ...
  <div style="margin-top:8px;font-size:10px;color:{_sig_col};opacity:.8">
    ...
    if not st.session_state.trade_active or st.session_state.trade_mode != "mockup":
        ...
        if st.button(_btn_label, use_container_width=True, type=_btn_type):
            # Use ATM option when signal active, else fall back to best_opt side
